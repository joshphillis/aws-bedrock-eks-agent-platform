"""
Research Agent — AWS Bedrock / EKS edition
Polls SQS research-tasks queue, calls Bedrock (Claude), publishes results to agent-results queue.
...
SYSTEM_PROMPT = """You are a research specialist. Given a research task, provide a thorough,
structured analysis based on your training knowledge.
...
async def process_message(body: dict) -> None:
    ...
    task   = body.get("task", "")
    logger.info("Processing research task job=%s", job_id)
    ...
    logger.info("Published research result job=%s confidence=%s",
                job_id, result.get("confidence"))


# ── SQS polling loop ──────────────────────────────────────────────────────────
async def poll_queue() -> None:
    logger.info("Research agent polling %s", SQS_RESEARCH_QUEUE_URL)
    loop = asyncio.get_running_loop()
    while True:
        try:
            resp = await loop.run_in_executor(
                ...
                lambda: sqs_client.receive_message(
                    QueueUrl=SQS_RESEARCH_QUEUE_URL,
...
# ── FastAPI application ───────────────────────────────────────────────────────
app = FastAPI(title="Research Agent", version="1.0.0", lifespan=lifespan)
...
async def ready():
    if not SQS_RESEARCH_QUEUE_URL or not SQS_RESULTS_QUEUE_URL:
